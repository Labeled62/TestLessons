...
fun main() {
    ...
    while (serverStatus != "success") {
        ...
        if (attempt < 3) {
            //эта ветка необязательна, смысле переприсваивать тоже значнеие processing нет фактически
...
fun simulateTestRuns() {
    for (i in 1..5) {
        when (i) {
            ...
            //в дз не описано, но лучше добавлять else в when для обработки остальных ситуаций(тут их нет конечно), тут например логично распечатать строку "Статус теста не определён"
        }
    }
}
...
fun evaluateTest(duration: Int, complexity: Int): String {
    //тут фактически немного описанные ветки отличаются от описанных в ДЗ, в в ветку else как раз лучше помещать случай который нельзя обработать по условиям
    //лучше так сформировать условия веток
    //return when {
    //    duration < 10 -> "Быстрый тест"
    //    duration <= 30 && complexity in 1..3 -> "Стандартный тест"
    //    duration <= 30 && complexity in 4..5 -> "Сложный тест"
    //    duration > 30 -> "Длительный тест"
